...
void W5500_GPIO_Configuration(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD|RCC_APB2Periph_AFIO|RCC_APB2Periph_GPIOB, ENABLE);

	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);
	
	/* W5500_RST引脚初始化配置(PC10) */
...
void Write_W5500_1Byte(unsigned short reg, unsigned char dat)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM1|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,1个字节数据长度,写数据,选择通用寄存器
...
void Write_W5500_2Byte(unsigned short reg, unsigned short dat)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM2|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,2个字节数据长度,写数据,选择通用寄存器
...
void Write_W5500_nByte(unsigned short reg, unsigned char *dat_ptr, unsigned short size)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(VDM|RWB_WRITE|COMMON_R);//通过SPI1写控制字节,N个字节数据长度,写数据,选择通用寄存器
...
void Write_W5500_SOCK_1Byte(SOCKET s, unsigned short reg, unsigned char dat)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM1|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,1个字节数据长度,写数据,选择端口s的寄存器
	SPI2_Send_Byte(dat);//写1个字节数据
...
void Write_W5500_SOCK_2Byte(SOCKET s, unsigned short reg, unsigned short dat)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM2|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,2个字节数据长度,写数据,选择端口s的寄存器
	SPI2_Send_Short(dat);//写16位数据
...
void Write_W5500_SOCK_4Byte(SOCKET s, unsigned short reg, unsigned char *dat_ptr)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM4|RWB_WRITE|(s*0x20+0x08));//通过SPI1写控制字节,4个字节数据长度,写数据,选择端口s的寄存器
...
unsigned char Read_W5500_1Byte(unsigned short reg)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM1|RWB_READ|COMMON_R);//通过SPI1写控制字节,1个字节数据长度,读数据,选择通用寄存器
...
unsigned char Read_W5500_SOCK_1Byte(SOCKET s, unsigned short reg)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM1|RWB_READ|(s*0x20+0x08));//通过SPI1写控制字节,1个字节数据长度,读数据,选择端口s的寄存器
...
unsigned short Read_W5500_SOCK_2Byte(SOCKET s, unsigned short reg)
{
	...
	SPI2_Send_Short(reg);//通过SPI1写16位寄存器地址
	SPI2_Send_Byte(FDM2|RWB_READ|(s*0x20+0x08));//通过SPI1写控制字节,2个字节数据长度,读数据,选择端口s的寄存器
...
void W5500_Init(void)
{
	...
	Write_W5500_2Byte(RTR_W, 0x07d0);

	//设置重试次数，默认为8次 
	//如果重发的次数超过设定值,则产生超时中断(相关的端口中断寄存器中的Sn_IR 超时位(TIMEOUT)置“1”)
	Write_W5500_1Byte(RCR_W,8);
