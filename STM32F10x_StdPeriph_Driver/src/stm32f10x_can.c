...
uint8_t CAN_Transmit(CAN_TypeDef* CANx, CanTxMsg* TxMessage)
{
    ...
    if (transmit_mailbox != CAN_TxStatus_NoMailBox)
  {
    ...
    if (TxMessage->IDE == CAN_Id_Standard) //标准帧
    {
      assert_param(IS_CAN_STDID(TxMessage->StdId));  
      CANx->sTxMailBox[transmit_mailbox].TIR |= ((TxMessage->StdId << 21) | \
                                                  TxMessage->RTR);
    }
    else								  									//扩展帧(原库函数有错误，发送时标准帧标示符没有发送出去)
    {
...
//                                                  TxMessage->RTR);
      ...
      TxMessage->ExtId <<= 3;
	  TxMessage->ExtId=TxMessage->ExtId|TxMessage->StdId;
      CANx->sTxMailBox[transmit_mailbox].TIR |= (TxMessage->ExtId | TxMessage->IDE | 
                                               TxMessage->RTR);
    }
